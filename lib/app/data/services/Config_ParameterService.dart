...
class Config_ParameterService{
  ...
  Future<void> setGlobalConfig(String userId) async {
    ...
    try {
      ...
      Utils.Log('Config_ParameterService.setGlobalConfig', 'catch: ${e.toString()}');
      return;
    }

    bool findVar(String x_variable_name) {
      Utils.Log('Config_ParameterService.setGlobalConfig', 'findVar: $x_variable_name');

      currentConfig_ParameterModel = listConfig_ParameterModel.firstWhere((element) =>
      element.key.toUpperCase() == (_prefixKey + x_variable_name).toUpperCase(), orElse: () => null);

      if (currentConfig_ParameterModel == null)
        Utils.Log('Config_ParameterService.setGlobalConfig', 'findVar not found');
      else
        Utils.Log('Config_ParameterService.setGlobalConfig', 'findVar Sucess with value : "${currentConfig_ParameterModel.value}"');
    ...
    if (globalConfig.activeRegisterLogApiUser){
      try {
        globalConfig.activeRegisterLogApiUser_QtdeChar = convertStringToInt(currentConfig_ParameterModel.value);
      }catch(e){
